...
// ── 연출 CSS ────────────────────────────────────────────────
const REVEAL_STYLES = `
  .reveal-overlay { position:fixed;inset:0;z-index:100;display:flex;align-items:center;justify-content:center;background:rgba(0,0,0,0.85);animation:overlayIn 0.3s ease; }
  @keyframes overlayIn{from{opacity:0}to{opacity:1}}
  .reveal-box{position:relative;display:flex;flex-direction:column;align-items:center;gap:16px;}
  .reveal-card-img{width:200px;height:280px;border-radius:16px;object-fit:cover;}
  .pentagram{position:absolute;width:260px;height:260px;top:50%;left:50%;transform:translate(-50%,-50%);pointer-events:none;}
  .pentagram svg{width:100%;height:100%;animation:pentaSpin 1.2s ease-out forwards;}
  @keyframes pentaSpin{0%{transform:rotate(0deg) scale(0.4);opacity:0}40%{opacity:1}80%{transform:rotate(360deg) scale(1.1);opacity:1}100%{transform:rotate(400deg) scale(1);opacity:0}}
  .r-card-in{animation:rCardIn 0.5s 0.9s ease both;}
  @keyframes rCardIn{from{transform:scale(0.6) rotate(-10deg);opacity:0}to{transform:scale(1) rotate(0deg);opacity:1}}
  .ur-shatter-wrap{position:relative;width:200px;height:280px;}
  .shard{position:absolute;background:#ffd700;border-radius:2px;animation:shardOut 0.6s ease-out forwards,shardIn 0.5s 1.0s ease forwards;opacity:0;}
  @keyframes shardOut{0%{opacity:1;transform:translate(0,0) rotate(0deg) scale(1)}100%{opacity:0;transform:translate(var(--tx),var(--ty)) rotate(var(--rot)) scale(0.3)}}
  @keyframes shardIn{0%{opacity:0;transform:translate(var(--tx),var(--ty)) rotate(var(--rot)) scale(0.3)}100%{opacity:1;transform:translate(0,0) rotate(0deg) scale(1)}}
  .ur-img-in{animation:urImgIn 0.6s 1.5s ease both;opacity:0;}
  @keyframes urImgIn{from{opacity:0;filter:brightness(3);transform:scale(1.1)}to{opacity:1;filter:brightness(1);transform:scale(1)}}
  .ur-glow-ring{position:absolute;inset:-20px;border-radius:20px;box-shadow:0 0 60px 20px rgba(255,215,0,0.6);animation:urGlow 2s 1.5s ease-in-out infinite alternate;pointer-events:none;}
  @keyframes urGlow{from{box-shadow:0 0 40px 10px rgba(255,215,0,0.5)}to{box-shadow:0 0 80px 30px rgba(255,215,0,0.9)}}
  .ssr-slash{position:absolute;top:50%;left:-10%;width:120%;height:3px;background:linear-gradient(90deg,transparent,#fff,#ffd700,#fff,transparent);transform:translateY(-50%) rotate(var(--angle));animation:slashMove 0.25s var(--delay) ease-out forwards;opacity:0;pointer-events:none;filter:blur(1px);}
  @keyframes slashMove{0%{opacity:0;transform:translateY(-50%) rotate(var(--angle)) scaleX(0)}30%{opacity:1}100%{opacity:0;transform:translateY(calc(-50% + var(--shift))) rotate(var(--angle)) scaleX(1.2)}}
  .crack-line{position:absolute;background:linear-gradient(var(--dir),transparent 0%,#ffd700 50%,transparent 100%);animation:crackForm 0.3s var(--cdelay) ease forwards,crackFade 0.5s 1.2s ease forwards;opacity:0;pointer-events:none;}
  @keyframes crackForm{from{opacity:0}to{opacity:0.8}}
  @keyframes crackFade{from{opacity:0.8;filter:brightness(2)}to{opacity:0;filter:brightness(1)}}
  .ssr-img-in{animation:ssrImgIn 0.7s 1.4s cubic-bezier(0.34,1.56,0.64,1) both;opacity:0;}
  @keyframes ssrImgIn{from{opacity:0;transform:scale(0.7);filter:brightness(4) saturate(0)}to{opacity:1;transform:scale(1);filter:brightness(1) saturate(1)}}
  .reveal-grade{font-size:11px;letter-spacing:4px;font-weight:700;padding:4px 14px;border-radius:20px;animation:fadeInUp 0.5s 1.8s ease both;}
  .reveal-name{font-size:22px;font-weight:700;letter-spacing:2px;animation:fadeInUp 0.5s 2.0s ease both;opacity:0;}
  .reveal-fortune{font-size:13px;text-align:center;max-width:260px;line-height:1.8;animation:fadeInUp 0.5s 2.2s ease both;opacity:0;color:#e8dcc8;}
  .reveal-close{margin-top:8px;background:transparent;border:1px solid rgba(255,255,255,0.2);color:#e8dcc8;font-size:13px;padding:8px 28px;border-radius:20px;cursor:pointer;letter-spacing:2px;animation:fadeInUp 0.5s 2.5s ease both;opacity:0;transition:border-color 0.2s;}
    ...
    <div className="reveal-overlay" onClick={onClose}>
      <div className="reveal-box" onClick={e=>e.stopPropagation()}>
        <div style={{position:"relative",width:200,height:280}}>
          <div className="pentagram"><svg viewBox="0 0 100 100"><polygon points="50,5 61,35 95,35 68,57 79,91 50,70 21,91 32,57 5,35 39,35" fill="none" stroke={cfg.color} strokeWidth="2" opacity="0.9"/></svg></div>
          <img src={imgSrc} className="reveal-card-img r-card-in" alt={card.name}/>
        </div>
        <div className="reveal-grade" style={{background:cfg.bg,color:cfg.color,border:`1px solid ${cfg.border}`}}>{cfg.label}</div>
...
function RevealUR({ card, imgSrc, cfg, onClose }) {
  const shards = Array.from({length:20},(_,i)=>({id:i,tx:`${(Math.random()-0.5)*300}px`,ty:`${(Math.random()-0.5)*300}px`,rot:`${(Math.random()-0.5)*720}deg`,w:`${20+Math.random()*40}px`,h:`${20+Math.random()*40}px`,top:`${Math.random()*80}%`,left:`${Math.random()*80}%`,delay:`${Math.random()*0.3}s`}));
  return (
    <div className="reveal-overlay" onClick={onClose}>
      <div className="reveal-box" onClick={e=>e.stopPropagation()}>
        <div className="ur-shatter-wrap">
          {shards.map(s=><div key={s.id} className="shard" style={{width:s.w,height:s.h,top:s.top,left:s.left,"--tx":s.tx,"--ty":s.ty,"--rot":s.rot,animationDelay:s.delay}}/>)}
          <div className="ur-glow-ring"/>
          <img src={imgSrc} className="reveal-card-img ur-img-in" alt={card.name} style={{position:"absolute",top:0,left:0}}/>
        </div>
        <div className="reveal-grade" style={{background:cfg.bg,color:cfg.color,border:`1px solid ${cfg.border}`}}>{cfg.label}</div>
        <div className="reveal-name" style={{color:cfg.color}}>{card.name}</div>
        <div className="reveal-fortune">✨ {card.fortune}</div>
...
function RevealSSR({ card, imgSrc, cfg, onClose }) {
  const slashes=[{angle:"-15deg",delay:"0s",shift:"-20px"},{angle:"10deg",delay:"0.2s",shift:"15px"},{angle:"-5deg",delay:"0.4s",shift:"-10px"}];
  const cracks=[{dir:"135deg",top:"20%",left:"30%",w:"2px",h:"60px",cdelay:"0.3s"},{dir:"160deg",top:"40%",left:"55%",w:"2px",h:"80px",cdelay:"0.5s"},{dir:"120deg",top:"10%",left:"60%",w:"2px",h:"50px",cdelay:"0.65s"},{dir:"145deg",top:"60%",left:"20%",w:"2px",h:"70px",cdelay:"0.45s"}];
  return (
    <div className="reveal-overlay" onClick={onClose}>
      <div className="reveal-box" onClick={e=>e.stopPropagation()}>
        <div style={{position:"relative",width:200,height:280}}>
          {slashes.map((s,i)=><div key={i} className="ssr-slash" style={{"--angle":s.angle,"--delay":s.delay,"--shift":s.shift}}/>)}
          {cracks.map((c,i)=><div key={i} className="crack-line" style={{top:c.top,left:c.left,width:c.w,height:c.h,"--dir":c.dir,"--cdelay":c.cdelay}}/>)}
...
export default function Gacha() {
  ...
  const [freeCount, setFreeCount] = useState(0); // 신규 무료 남은 횟수
  const [pityCount, setPityCount] = useState(0); // 천장 카운터
  ...
  const doGacha = (count = 1) => {
    ...
    setTimeout(() => {
      ...
      const drawn = drawCard(newPity);
      setPityCount(newPity);
      localStorage.setItem("gacha_pity", String(newPity));
      setCard(drawn);
      setPhase("result");
      setShowReveal(true);
    ...
    <>
      ...
      <style>{`
        *,*::before,*::after{box-sizing:border-box;margin:0;padding:0;}
        ...
        .app{min-height:100vh;background:radial-gradient(ellipse at 20% 20%,rgba(120,40,140,0.18) 0%,transparent 60%),radial-gradient(ellipse at 80% 80%,rgba(40,20,80,0.25) 0%,transparent 60%),radial-gradient(ellipse at 50% 50%,rgba(7,5,15,1) 0%,rgba(7,5,15,1) 100%);color:#e8dcc8;padding:40px 20px 60px;display:flex;flex-direction:column;align-items:center;}
      ...
      <div className="app">
        ...
        <div className="yeonhwa-wrap">
          <img src={yeonhwaImg} alt="연화" className={`yeonhwa-img ${phase}`} onError={()=>{}}/>
