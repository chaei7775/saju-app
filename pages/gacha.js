...
// ── 등급별 뒤집기 연출 ──────────────────────────────────────
function CardRevealEffect({ card, onClose }) {
  const cfg = GRADE_CONFIG[card.grade];
  const imgSrc = `${GITHUB_BASE}/${IMAGE_MAP[card.id]}`;

  const gradeEffects = {
    N:   { bg: "rgba(0,0,0,0.88)", particle: null },
    R:   { bg: "rgba(0,5,0,0.88)", particle: "✦" },
    SR:  { bg: "rgba(5,0,20,0.92)", particle: "✦" },
    SSR: { bg: "rgba(20,0,0,0.92)", particle: "🔥" },
    UR:  { bg: "rgba(10,8,0,0.95)", particle: "✨" },
  };
  const fx = gradeEffects[card.grade];
      ...
      <div onClick={e => e.stopPropagation()} style={{
        ...
        {card.grade === "UR" && (
          <div style={{
            ...
            animation: "urAura 1.5s ease-in-out infinite alternate",
          }}/>
        )}
        {card.grade === "SSR" && (
          <div style={{
        ...
        <div style={{
          ...
          animation: "cardRevealPop 0.6s cubic-bezier(0.34,1.56,0.64,1) both",
        }}>
          {/* UR 골드링 */}
          {card.grade === "UR" && (
            <div style={{
              ...
              borderRadius: 20,
              boxShadow: "0 0 60px 20px rgba(255,215,0,0.5)",
              animation: "urGlow 1.5s ease-in-out infinite alternate",
              pointerEvents: "none",
            }}/>
          )}
          {/* SSR 슬래시 */}
          {card.grade === "SSR" && ["-15deg","10deg","-5deg"].map((a,i) => (
            <div key={i} style={{
              position: "absolute", top: "50%", left: "-10%",
              width: "120%", height: 2,
              background: "linear-gradient(90deg,transparent,#fff,#f87171,transparent)",
              transform: `translateY(-50%) rotate(${a})`,
              animation: `slashAnim 0.25s ${i*0.15}s ease-out both`,
              pointerEvents: "none",
            }}/>
          ))}
          <img
        ...
        <div style={{
          ...
          padding: "4px 16px", letterSpacing: 4,
        ...
        {/* 운세 */}
        <div style={{
          fontSize: 13, color: "#e8dcc8", letterSpacing: 1,
        ...
        <button onClick={onClose} style={{
          ...
          animation: "fadeInUp 0.5s 0.8s ease both", opacity: 0,
          animationFillMode: "forwards",
        }}>閉じる</button>
      </div>
    </div>
...
// ── 10장 카드 선택 연출 ──────────────────────────────────────
function TenCardSelect({ cards, onSelect }) {
  const [chosen, setChosen] = useState(null);
  const [flipping, setFlipping] = useState(false);
  const [dismissed, setDismissed] = useState(false);

  const handlePick = (idx) => {
    if (chosen !== null || flipping) return;
    setFlipping(true);
    setChosen(idx);
    // 나머지 카드 사라짐 → 선택 카드 뒤집기 → 결과
    setTimeout(() => {
      setDismissed(true);
      setTimeout(() => onSelect(cards[idx]), 400);
    }, 600);
  };
    ...
    <div style={{
          ...
          return (
            <div
              ...
              style={{
                ...
                animation: `cardFloat 0.5s ${idx * 0.05}s ease both`,
                opacity: dismissed && isOther ? 0 : 1,
                transform: dismissed && isOther ? "scale(0.8)" : "scale(1)",
            ...
            >
              <div style={{
                ...
                transform: isChosen && dismissed ? "rotateY(180deg)" : "rotateY(0deg)",
                ...
                <div style={{
                  ...
                  border: `1px solid ${isChosen ? "rgba(201,169,110,0.6)" : "rgba(167,139,250,0.25)"}`,
                  display: "flex", alignItems: "center", justifyContent: "center",
                  boxShadow: isChosen ? "0 0 20px rgba(201,169,110,0.4)" : "0 0 8px rgba(167,139,250,0.1)",
                  transition: "border-color 0.2s, box-shadow 0.2s",
                }}>
                  <div style={{
                    fontSize: 18, opacity: isChosen ? 1 : 0.5,
      ...
      {chosen === null && (
        <div style={{
          ...
          color: "rgba(154,138,112,0.5)", letterSpacing: 2,
          animation: "fadeInUp 0.5s 1s ease both", opacity: 0,
          animationFillMode: "forwards",
        }}>
          운명의 카드를 선택하세요
        </div>
...
// ResultSummary 제거됨 — 1장 선택 후 CardRevealEffect로 바로 연출
...
export default function Gacha() {
  const [phase, setPhase] = useState("ready"); // ready | spinning | selecting | reveal
  ...
  const doGacha = () => {
    ...
    setTimeout(() => {
      ...
      setYeonhwaMsg("一枚選んでください…");
    }, 800);

    setTimeout(() => {
      // 실제 결과 1장 + 더미 9장 = 총 10장
      const newPity = pityCount + 1;
      const realCard = drawCard(newPity);
      setPityCount(newPity);
      localStorage.setItem("gacha_pity", String(newPity));
      localStorage.setItem("gacha_last_date", getToday());
      setUsedToday(true);

      // 10장 배열: 실제 카드를 랜덤 위치에 섞기
      const dummies = Array.from({ length: 9 }, () => {
        const p = CARDS.filter(c => c.grade === "N");
        return p[Math.floor(Math.random() * p.length)];
      });
      const allCards = [...dummies, realCard];
      // 셔플
      ...
      // 실제 카드 위치 기록
      const realIdx = allCards.findIndex(c => c === realCard);
      setDrawnCards({ cards: allCards, realIdx });
      setPhase("selecting");
    }, 1600);
  };
    ...
    <>
      ...
      <style>{`
        *,*::before,*::after{box-sizing:border-box;margin:0;padding:0;}
        ...
        @keyframes cardRevealPop{from{opacity:0;transform:scale(0.5) rotate(-5deg)}to{opacity:1;transform:scale(1) rotate(0deg)}}
        @keyframes urGlow{from{box-shadow:0 0 40px 10px rgba(255,215,0,0.4)}to{box-shadow:0 0 80px 30px rgba(255,215,0,0.8)}}
        @keyframes urAura{from{opacity:0.5}to{opacity:1}}
        @keyframes slashAnim{0%{opacity:0;transform:translateY(-50%) rotate(var(--a,0deg)) scaleX(0)}50%{opacity:1}100%{opacity:0;transform:translateY(-50%) rotate(var(--a,0deg)) scaleX(1.2)}}
      `}</style>
      ...
      <div className="app">
        ...
        {/* 10장 카드 선택 연출 */}
        {phase === "selecting" && drawnCards.cards && (
          <TenCardSelect cards={drawnCards.cards} onSelect={handleSelect} />
        )}

        {/* 선택한 카드 화려한 연출 */}
        {phase === "reveal" && selectedCard && (
          <CardRevealEffect card={selectedCard} onClose={reset} />
        )}
